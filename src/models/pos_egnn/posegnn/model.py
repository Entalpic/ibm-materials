...
class PosEGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        data.pos, data.box, data.displacements = get_symmetric_displacement(
            data.pos, data.box, data.num_graphs, data.batch
        )

        (
            data.cutoff_edge_index,
            data.cutoff_edge_distance,
            data.cutoff_edge_vec,
            data.cutoff_shifts_idx,
        ) = self.distance(data.pos, data.box, data.batch)
